			...
			<tbody>
				...
				if (count($filenames) != 0) { 
					foreach ($filenames as $file) {
						// Skip system files and sensitive file types
						if ($file == "." || $file == "..") {
							continue;
						}
						
						// Skip hidden files and sensitive file types
						$ext = strtolower(pathinfo($file, PATHINFO_EXTENSION));
						$isSensitive = in_array($ext, ['php', 'phtml', 'php3', 'php4', 'php5', 'php7', 'phps', 'inc']);
						$isHidden = substr($file, 0, 1) == '.' || $file == '.htaccess' || $file == 'web.config' || $file == '.gitignore' || $file == '.gitkeep';
						...
						if (is_dir($path . $file)) {
							...
							// File
							// Additional check for PHP files in the file processing section too
							$fileExt = strtolower(pathinfo($file, PATHINFO_EXTENSION));
							if (in_array($fileExt, ['php', 'phtml', 'php3', 'php4', 'php5', 'php7', 'phps', 'inc'])) {
				...
				if (count($filesSorted) != 0) {		   
					foreach ($filesSorted as $file) {
						...
						// Skip PHP files entirely (they should already be filtered above, but just in case)
						if (in_array($ext, ['php', 'phtml', 'php3', 'php4', 'php5', 'php7', 'phps', 'inc'])) {
